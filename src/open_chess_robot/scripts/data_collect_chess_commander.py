...
import time
from ocr_runtime.paths import resource_path
...
class ChessCommanderFen(ChessCommander):
    ...
    def load_chess_games(self, folder=None):
        games = list()
        folder = (
            Path(folder)
            if folder is not None
            else resource_path("scripts", "data", "games", "Adams")
        )
        rospy.loginfo(f"loading at path: {folder}")
        games_list = glob.glob(str(folder / "*.csv"))
        for file in np.sort(games_list):
            games.append(
                np.genfromtxt(file, delimiter=",", dtype=str)
            )
...
if __name__ == "__main__":
    try:
        ...
        my_chess.replay(path=None, num_step=1000, execution=True, start_id=4)
