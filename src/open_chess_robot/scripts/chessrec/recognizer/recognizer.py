...
class ChessRecognizer:
    ...
    def __init__(self, classifiers_folder: Path = URI("models://")):
        ...
        self._occupancy_cfg, self._occupancy_model = self._load_classifier(
            classifiers_folder / "occupancy_classifier", epoch="")
        ...
        self._pieces_cfg, self._pieces_model = self._load_classifier(
            classifiers_folder / "piece_classifier", epoch="")
        ...
        # Per-square softmax confidence of the most recent classification, aligned
        # to ``self._squares``. piece_confidence is 0 for squares classified empty.
        self.occupancy_confidence = np.zeros(len(self._squares))
        self.piece_confidence = np.zeros(len(self._squares))
    ...
    def _classify_occupancy(self, img: np.ndarray, turn: chess.Color, corners: np.ndarray) -> np.ndarray:
        ...
        logits = self._occupancy_model(square_imgs)
        probs = torch.softmax(logits, dim=-1)
        confidence, predicted = probs.max(dim=-1)
        occupied_index = self._occupancy_cfg.DATASET.CLASSES.index("occupied")
        occupancy = (predicted == occupied_index).cpu().numpy()
        self.occupancy_confidence = confidence.cpu().numpy()
        return occupancy
    ...
    def _classify_pieces(self, img: np.ndarray, turn: chess.Color, corners: np.ndarray, occupancy: np.ndarray) -> np.ndarray:
        ...
        logits = self._pieces_model(piece_imgs)
        probs = torch.softmax(logits, dim=-1)
        confidence, predicted = probs.max(dim=-1)
        pieces = self._piece_classes[predicted.cpu().numpy()]
        all_pieces = np.full(len(self._squares), None, dtype=np.object_)
        all_pieces[occupancy] = pieces
        piece_confidence = np.zeros(len(self._squares))
        piece_confidence[occupancy] = confidence.cpu().numpy()
